...
void ED_CallSpawn (edict_t *ent)
{
	...
	if (!strncmp(ent->classname, "monster_", 8))//FREDZ remove monsters, less spam on q2 maps
	{
		G_FreeEdict(ent);
		return;
	}
/*
	if (!strcmp(ent->classname, "hmg_mod_cooling") || !strncmp(ent->classname, "pistol_mod_", 11)//FREDZ remove this if you want to use mods
		)
	{
		G_FreeEdict(ent);
		return;
...
*/
	...
	if (!strcmp(ent->classname, "weapon_bfg"))
		ent->classname = "weapon_flamethrower";
	//Armor
	if (!strcmp(ent->classname, "item_armor_body"))
		ent->classname = "item_armor_jacket_heavy";
	if (!strcmp(ent->classname, "item_armor_combat"))
		ent->classname = "item_armor_legs_heavy";
//	if (!strcmp(ent->classname, "item_armor_jacket"))//FREDZ there is already a jacket in KP
//		ent->classname = "item_armor_helmet_heavy";
/*	if (!strcmp(ent->classname, "item_armor_shard"))
	{
		int rnd = rand() % 3;
		if (rnd < 1)
			ent->classname = "item_armor_helmet";
		else if (rnd < 2)
			ent->classname = "item_armor_legs";
		else
			ent->classname = "item_armor_helmet_heavy";
//			ent->classname = "item_armor_jacket";
	...
	if (!strcmp(ent->classname, "item_health_large"))
		ent->classname = "item_health_lg";
	if (!strcmp(ent->classname, "item_health"))
		ent->classname = "item_health_sm";
...
//hypov8 ammo clip in gun
"if 2 "
"	xv	173 "
"	num 2 18" //STAT_CLIP 18
"endif "
...
/*	gi.modelindex ("#w_blaster.md2");
	...
	//Kingpin FREDZ thx to Hypov8 for model
	gi.modelindex("#w_pipe.md2");				// CLIP_NONE
	gi.modelindex("#w_pistol.md2");				// CLIP_PISTOL
	gi.modelindex("#w_shotgun.md2");			// CLIP_SHOTGUN
	gi.modelindex("#w_tommygun.md2");			// CLIP_TOMMGUN
	gi.modelindex("#w_heavy machinegun.md2");	// CLIP_SLUGS
	gi.modelindex("#w_grenade launcher.md2");	// CLIP_GRENADES
	gi.modelindex("#w_bazooka.md2");			// CLIP_ROCKETS
	gi.modelindex("#w_flamethrower.md2");		// CLIP_FLAMEGUN
