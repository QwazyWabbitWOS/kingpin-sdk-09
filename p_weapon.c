...
qboolean Pickup_Weapon (edict_t *ent, edict_t *other)
{
	...
	//hypov8  SPistol
	if (/*other->client->pers.weapon*/ ent->item == FindItem ("SPistol"))
...
void ChangeWeapon (edict_t *ent)
{
	...
	//hypov8
	//308 hmg mod
	if (ent->client->clip_index == CLIP_SLUGS)
	{
		if (ent->client->pers.pistol_mods & WEAPON_MOD_COOLING_JACKET)
			ent->client->ps.gunindex = gi.modelindex("models/weapons/v_hmgcool/hmgcool.md2");
	}
	...
	if (ent->client->clip_index == CLIP_PISTOL)
	{
		if (!strcmp(ent->client->pers.weapon->pickup_name, "SPistol"))
		{
			if (ent->client->pers.pistol_mods & WEAPON_MOD_DAMAGE)
				ent->client->ps.gunindex = gi.modelindex("models/weapons/silencer/magnum.md2");
		}
		else if (!strcmp(ent->client->pers.weapon->pickup_name, "Pistol"))
		{
			if (ent->client->pers.pistol_mods & WEAPON_MOD_DAMAGE)
				ent->client->ps.gunindex = gi.modelindex("models/weapons/v_colt/magnum.md2");
		}
	}
...
void Blackjack_Hit (edict_t *ent, vec3_t vorigin, int damage)
{
	...
	fire_blackjack(ent, start, forward, damage, kick, MOD_BLACKJACK);

	/*
	gi.WriteByte(svc_muzzleflash);
	gi.WriteShort(ent - g_edicts);
	gi.WriteByte(MZ_ITEMRESPAWN);//FREDZ was MZ_BLACKJACK_SWING
	gi.multicast(ent->s.origin, MULTICAST_PVS);*/
	gi.sound(ent, CHAN_VOICE, gi.soundindex("weapons/melee/swing.wav"), 1, ATTN_NORM, 0);//FREDZ kingpin sound
}
...
void Crowbar_Hit (edict_t *ent, vec3_t vorigin, int damage)
{
	...
	fire_crowbar(ent, start, forward, damage, kick, MOD_CROWBAR);
/*
	gi.WriteByte(svc_muzzleflash);
	gi.WriteShort(ent - g_edicts);
	gi.WriteByte(MZ_ITEMRESPAWN);//FREDZ was MZ_CROWBAR_SWING
	gi.multicast(ent->s.origin, MULTICAST_PVS);*/
	gi.sound(ent, CHAN_VOICE, gi.soundindex("weapons/melee/swing.wav"), 1, ATTN_NORM, 0);//FREDZ kingpin sound
}
...
void Pistol_Fire (edict_t *ent, vec3_t vorigin, int damage)
{
	...
	if (ent->client->pers.pistol_mods & WEAPON_MOD_DAMAGE)
	{
		gi.WriteByte(svc_muzzleflash);
		gi.WriteShort(ent - g_edicts);
		//gi.WriteByte (MZ_PISTOL_DAMAGE_MOD | is_silenced);//KP
		gi.WriteByte(MZ_CHAINGUN1 | is_silenced); //hypov8 sounds
		gi.multicast(ent->s.origin, MULTICAST_PVS);
	}
	else
	{
		gi.WriteByte(svc_muzzleflash);
		gi.WriteShort(ent - g_edicts);
		//gi.WriteByte (MZ_PISTOL | is_silenced);//KP
		gi.WriteByte(MZ_MACHINEGUN | is_silenced);//FREDZ
		gi.multicast(ent->s.origin, MULTICAST_PVS);
	}
	*/
	if (ent->client->pers.pistol_mods & WEAPON_MOD_DAMAGE)//FREDZ kingpin sound
	{
		gi.sound(ent, CHAN_VOICE, gi.soundindex(va("weapons/pistol/pistolf%i.wav", (rand() % 4) + 1)), 1, ATTN_NORM, 0);
	}
	else
	{
		gi.sound(ent, CHAN_VOICE, gi.soundindex(va("weapons/pistol/pistola%i.wav", (rand() % 4) + 1)), 1, ATTN_NORM, 0);
	}
...
void Weapon_Pistol_Fire (edict_t *ent)
{
	...
	else if (ent->client->ps.gunframe == 10)
	{
		if (!ent->client->pers.weapon_clip[ent->client->clip_index])
		{
			...
			ent->client->ps.gunframe++;	
			if (ent->client->pers.pistol_mods & WEAPON_MOD_RELOAD) //hypov8 pistol mods. no sound?
...
void SPistol_Fire (edict_t *ent, vec3_t vorigin, int damage)
{
	...
	gi.WriteByte (svc_muzzleflash);
	gi.WriteShort (ent-g_edicts);
//	gi.WriteByte (MZ_SPISTOL | is_silenced);//KP
	gi.WriteByte(MZ_MACHINEGUN|MZ_SILENCED| is_silenced); //hypov8 todo: use sound slot from other wep
	gi.multicast (ent->s.origin, MULTICAST_PVS);
	*/
	gi.sound(ent, CHAN_VOICE, gi.soundindex("weapons/pistol/silencer.wav"), 1, ATTN_NORM, 0);//FREDZ kingpin sound
...
// gi.dprintf ("silencer shots left: %d\n", ent->client->pers.silencer_shots);
	...
	{
		//hypov8 todo md2 hide mesh of screen/shrink vertex. add null frames to pistol
	...
	if (ent->client->pers.pistol_mods & WEAPON_MOD_DAMAGE)
		ent->client->ps.gunindex = gi.modelindex("models/weapons/silencer_mdx/magnum.md2");
...
void Tommygun_Fire (edict_t *ent)
{
	...
	gi.multicast (ent->s.origin, MULTICAST_PVS);
	*/
	gi.sound(ent, CHAN_VOICE, gi.soundindex(va("weapons/machinegun/machgf%ib.wav", (rand() % 5) + 1)), 1, ATTN_NORM, 0);//FREDZ kingpin sound
...
*/
	...
	fire_flamethrower(ent, start, forward, damage, 380);
...
void shotgun_fire (edict_t *ent)
{
	...
	gi.multicast (ent->s.origin, MULTICAST_PVS);
	*/
	gi.sound(ent, CHAN_VOICE, gi.soundindex("weapons/shotgun/shotgf1b.wav"), 1, ATTN_NORM, 0);//FREDZ kingpin sound
...
//		gi.WriteByte (MZ_BARMACHINEGUN2 | is_silenced);
	...
	gi.sound(ent, CHAN_VOICE, gi.soundindex("weapons/hmg/hmg.wav"), 1, ATTN_NORM, 0);//FREDZ kingpin sound
...
void weapon_grenadelauncher_fire (edict_t *ent)
{
	...
	gi.multicast (ent->s.origin, MULTICAST_PVS);*/
	gi.sound(ent, CHAN_VOICE, gi.soundindex("weapons/grenade_launcher/gl_fire.wav"), 1, ATTN_NORM, 0);//FREDZ kingpin sound
...
void weapon_rocketlauncher_fire (edict_t *ent)
{
	...
	gi.sound(ent, CHAN_VOICE, gi.soundindex("weapons/rocket_launcher/rl_fire.wav"), 1, ATTN_NORM, 0);//FREDZ kingpin sound
