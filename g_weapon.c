...
void flame_explode(edict_t *self)
{
	if (self->s.frame == 2)
	{
		G_FreeEdict(self);
...
void flame_touch(edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	if (other->takedamage)
	{
		T_Damage(other, self, self->owner, self->velocity, self->s.origin, plane->normal, self->dmg, 1, DAMAGE_ENERGY, MOD_FLAMETHROWER);//FREDZ MOD_FLAMETHROWER was MOD_WOLF_FLAME
	}
	T_RadiusDamage(self, self->owner, 40, other, 40, MOD_FLAMETHROWER);//FREDZ MOD_FLAMETHROWER was MOD_WOLF_FLAMEBURN
	...
	VectorClear(self->velocity);
	self->s.modelindex = gi.modelindex("sprites/firewall.sp2");
	self->s.frame = 0;
	self->s.sound = 0;
//	self->s.effects = EF_FLAG1;
	self->s.renderfx = RF_TRANSLUCENT;
	self->think = flame_explode;
	self->nextthink = level.time + FRAMETIME;
	self->enemy = other;
	gi.linkentity(self);
}
...
void FlameJunc_Think(edict_t *self)
{
	...
	self->s.frame++;
	self->nextthink = level.time + FRAMETIME;
	T_RadiusDamage(self, self->owner, 32, self->owner, 40, MOD_FLAMETHROWER);//FREDZ MOD_FLAMETHROWER was MOD_WOLF_FLAMEBURN
}

void fire_flamethrower(edict_t *self, vec3_t start, vec3_t dir, int damage, int speed)
{
	edict_t	*junc;
	trace_t	tr;
	...
	VectorCopy(start, junc->s.old_origin);
	vectoangles(dir, junc->s.angles);
	VectorScale(dir, speed, junc->velocity);
	junc->svflags = SVF_DEADMONSTER;
	junc->movetype = MOVETYPE_FLYMISSILE;
	junc->clipmask = MASK_SHOT;
	junc->solid = SOLID_BBOX;
//	junc->s.effects = EF_FLAG1;
	junc->s.renderfx = RF_TRANSLUCENT;

	VectorClear(junc->mins);
	VectorClear(junc->maxs);

	junc->s.modelindex = gi.modelindex("sprites/fthrow1.sp2");
	junc->owner = self;
	junc->touch = flame_touch;
	junc->nextthink = level.time + FRAMETIME;
	junc->think = FlameJunc_Think;
	junc->dmg = damage;
	junc->classname = "flame";

	gi.linkentity(junc);

//	if (self->client)
//		check_dodge(self, junc->s.origin, dir, speed);

	tr = gi.trace(self->s.origin, NULL, NULL, junc->s.origin, junc, MASK_SHOT);
	if (tr.fraction < 1.0)
	{
		VectorMA(junc->s.origin, -12, dir, junc->s.origin);
		junc->touch(junc, tr.ent, NULL, NULL);
	}
}
